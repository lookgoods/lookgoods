...
export default class ContentView extends Component {
	...
	render() {
		const { contentList, contentMessage, isEditButton, setEditButton, handleChangeTextBox, deleteContentBox } = this.props
		return (
			<View>
					...
					<View style={{ marginBottom: 5, flexDirection: 'row' }}>
						...
						<View style={{ flex: 1, justifyContent: 'flex-end', flexDirection: 'row', alignItems: 'center'}}>
							{ contentList.length === 0 ?
								...
								<TouchableOpacity style={{ marginLeft: 24, flexDirection: 'row' }} onPress={() => setEditButton()}>
									<IconFontAwesome name='edit' size={18} />
									<Text style={{ fontSize: 15, color: colors.gray, marginLeft: 5, marginRight: 15 }}>{ isEditButton ? 'Done' : 'Edit' }</Text>
								</TouchableOpacity>}
						</View>
					</View>
				</View>
				...
				{contentList.map((item, key) => 
					<View key={key} >
						{ item.type === 'text' && (
							<View>
								{ isEditButton && (
									<TouchableOpacity style={{ top: 25, marginTop: -20, alignSelf: 'flex-end', right: 6, zIndex: 1 }} onPress={() => deleteContentBox(key)}>
